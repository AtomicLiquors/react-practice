...
const ERRORS = {
 ...
 INPUT: 'Please add an input video to the preroll folder',
};
...
*/
/*Hi Raj, you would just need to add an additional try catch if you want to gracefully skip:
        for (const i of inputFiles) {
            const iPath = join(FOLDERS.INPUT, i);
            const stat = await fsPromises.stat(iPath);
